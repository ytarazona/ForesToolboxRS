#' Gain and loss in the land cover classification
#'
...
coverChange <- function(x, y) {
  ...
  loss_ha <- lapply(
    1:max(raster::values(x), na.rm = TRUE),
    function(i) {
      sum_ha <- sum(x[x == i]) * raster::res(x)[1]^2 / 10000
    }
  )

  gain_ha <- lapply(
    1:max(raster::values(y), na.rm = TRUE),
    function(j) {
      sum_ha <- sum(y[y == j]) * raster::res(y)[1]^2 / 10000
    }
  )
