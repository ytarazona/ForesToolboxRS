...
plot.pvts <- function(x, ylab, type, col, lwd, main, ...) {
  ...
  if (!is.na(x$Breakpoint[1])) {
    ...
    legend(m, max(x$Ts) * 0.65, c("Ts", "Lower limit", "Breakpoint Not detected"), inset = .02, cex = 1, lty = c(1, 2, 1), lwd = c(2, 2, 5), col = c("black", "red", "slateblue1"), bty = "n", y.intersp = 0.3, x.intersp = 0.2)
  }
}
